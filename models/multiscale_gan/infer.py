#!/usr/bin/env python3
"""
Multiscale GAN inference — saves side-by-side visual strips.

Strip columns: person | cloth | agnostic | coarse | refined (GAN)

Usage:
  python models/multiscale_gan/infer.py --n 8
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Multiscale GAN inference")
    p.add_argument("--n",        type=int, default=8)
    p.add_argument("--data",     default=str(ROOT / "dataset" / "test" / "tensors"))
    p.add_argument("--save",     default=str(ROOT / "results" / "multiscale_gan"))
