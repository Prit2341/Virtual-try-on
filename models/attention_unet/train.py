#!/usr/bin/env python3
"""
Two-stage training: AttentionWarpNet → AttentionTryOnNet.

Usage:
  python models/attention_unet/train.py --stage both --data dataset/train/tensors
  python models/attention_unet/train.py --stage warp
  python models/attention_unet/train.py --stage tryon
"""
...
def _setup_logger(log_path: Path) -> logging.Logger:
    ...
    logger = logging.getLogger("attention_unet")
...
def train_tryon(args, logger: logging.Logger) -> None:
    ...
    if warp_best.exists():
        ...
        logger.info(f"[tryon] loaded AttentionWarpNet from {warp_best}")
    else:
    ...
    opt       = torch.optim.Adam(tryon_net.parameters(), lr=args.lr, betas=(0.5, 0.999))
    scaler    = GradScaler(enabled=(DEVICE == "cuda"))
    vgg       = VGGLoss().to(DEVICE)
    l1_fn     = nn.L1Loss()
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch in loader:
            ...
            scaler.scale(loss).backward()
            scaler.unscale_(opt)
            torch.nn.utils.clip_grad_norm_(tryon_net.parameters(), max_norm=1.0)
            scaler.step(opt)
            scaler.update()
            epoch_loss += loss.item()
        ...
        _save_checkpoint({"epoch": epoch, "model": tryon_net.state_dict(),
                          "opt": opt.state_dict(), "loss": avg_loss}, ep_path)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Attention U-Net training")
    ...
    p.add_argument("--ckpt-dir", default=str(ROOT / "checkpoints" / "attention_unet"),
                   dest="ckpt_dir")
    p.add_argument("--log-dir",  default=str(ROOT / "logs" / "attention_unet"),
                   dest="log_dir")
    return p.parse_args()
...
def main() -> None:
    ...
    logger.info(f"Attention U-Net training  stage={args.stage}  device={DEVICE}")
