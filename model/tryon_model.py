...
class UpBlockNoSkip(nn.Module):
    """Final upsample without skip connection."""
...
class TryOnNet(nn.Module):
    ...
    Input (43ch): agnostic(3) + warped_cloth(3) + warped_mask(1)
                  + pose(18) + parse_onehot(18)
    Output:       RGB image (3, H, W) in [-1, 1]

    Improvements over v1:
      - 43-channel input (warped mask + parse one-hot for better guidance)
      - 5 encoder blocks (deeper)
      - Concat skip connections (preserves detail)
      - Self-attention at bottleneck
      - Full-resolution output via final UpBlockNoSkip
    """

    def __init__(self, in_channels=43, ngf=64):
        super().__init__()
        ...
        self.d5 = UpBlockNoSkip(ngf, ngf // 2)          # → 32  (full res, no skip)
